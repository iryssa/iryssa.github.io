...
// Save the ground array to a cookie.
function saveGround() {
  try {
    const data = JSON.stringify(ground);
    // Save a cookie with an expiry of 365 days.
    setCookie("snowpackGround", data, 365);
  } catch (e) {
    console.warn("Could not save snowpack to cookie", e);
  }
}

// Try to load the stored ground from cookie; if none exists, create a new array.
let savedData = getCookie("snowpackGround");
let ground;
if (savedData) {
  try {
    ground = JSON.parse(savedData);
    // If stored array length doesn't match current width, reset.
    if (!Array.isArray(ground) || ground.length !== W) {
...
let newAccumulation = false; // Flag for new snowfall this frame.
...
// Create the array of snowflakes.
for (let i = 0; i < maxFlakes; i++) {
  flakes.push(new Snowflake());
}

// When a snowflake lands, deposit its mass to the ground array.
...
// Update and smooth the ground when new snow is added.
function updateGround() {
  ...
  // Avalanche collapse: adjust differences too steep.
  ...
  // Save updated ground to cookie.
  saveGround();
}

// Draw the snow surface along the bottom using a smooth cubic Bézier curve.
function drawGround() {
  // 1. Sample the ground at fixed intervals.
  const samples = [];
  const step = 8;
  samples.push({ x: 0, y: H - ground[0] });
  for (let x = step; x < ground.length; x += step) {
    samples.push({ x: x, y: H - ground[x] });
  }
  samples.push({ x: W, y: H - ground[ground.length - 1] });

  // 2. Smooth the sample points.
  const smoothSamples = [];
  smoothSamples.push(samples[0]);
  for (let i = 1; i < samples.length - 1; i++) {
    const avgY = (samples[i - 1].y + samples[i].y + samples[i + 1].y) / 3;
    smoothSamples.push({ x: samples[i].x, y: avgY });
  }
  smoothSamples.push(samples[samples.length - 1]);

  // 3. Draw the curve using cubic Bézier segments.
  ctx.beginPath();
  ctx.moveTo(0, H);
...
// Main animation loop: update and render the snowflakes and ground.
function drawSnow() {
  ctx.clearRect(0, 0, W, H);
  ctx.fillStyle = "#000";
  ctx.fillRect(0, 0, W, H);

  // Process and render each snowflake.
